...
class location:
    def __init__(self,dis_pathloss,ble_location,delta):
        ...
        for idx in ble_location:
            self.node_locations.append([float(ble_location[idx]['lat'])/delta['lat'], float(ble_location[idx]['lon'])/delta['lon'], float(ble_location[idx]['alt'])/delta['alt']])
            pos[idx]=cnt
            cnt=cnt+1
    ...
    def estimate(self):
        ...
        ret={}
        ret['lat']=float(self.gbest_pos[0])*self.delta['lat']
        ret['lon']=float(self.gbest_pos[1])*self.delta['lon']
